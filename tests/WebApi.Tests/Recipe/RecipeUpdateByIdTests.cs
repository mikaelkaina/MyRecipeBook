...
public class RecipeUpdateByIdTests : IClassFixture<CustomWebApplicationFactory>
{
    private const string REQUEST_URI = "/api/recipes";
    private readonly HttpClient _httpClient;
    private readonly CustomWebApplicationFactory _factory;

    public RecipeUpdateByIdTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    public async Task Success()
    {
        ...
        var response = await _httpClient.PutAsJsonAsync($"{REQUEST_URI}/{recipe.Id}", request);
    ...
    public async Task Validate_ShouldBeAnErrorResponse_WhenRecipeNotFound(string culture)
    {
        ...
        var response = await _httpClient.PutAsJsonAsync($"{REQUEST_URI}/{Guid.CreateVersion7()}", request);
